...
function saveFile() {
	const fs = require('fs');
	fs.writeFileSync('powerManage.txt',String(variable_udp));
}
...
function timer_upload_action() {
	...
	if(Tvalue != pre_value){
		pre_value = Tvalue;
		var con = {value: Tvalue};
		for (var i = 0; i < upload_arr.length; i++) {
		    if (upload_arr[i].ctname == 'sensor') {
		        var cin = {ctname: upload_arr[i].ctname, con: con};
			console.log(variable_udp);
			saveFile();
